...
// Componente principal del header del recepcionista
@Component({
  selector: 'app-header-recepcionista',
  standalone: true,
  imports: [CommonModule, RouterModule, MatButtonModule, MatDialogModule],
  templateUrl: './header-recepcionista.html',
  styleUrls: ['./header-recepcionista.css']
})
export class HeaderRecepcionistaComponent implements OnInit {
  isMobileMenuOpen = false;
  currentPageTitle = 'Harmony Gym';
  currentPageSubtitle = 'Sistema de recepción';
  
  // Definir los títulos para cada ruta DEL RECEPCIONISTA
  private pageTitles: { [key: string]: { title: string; subtitle: string } } = {
    '/recepcionista': {
      title: 'Dashboard Recepción',
      subtitle: 'Panel principal de gestión'
    },
    '/pagos': {
      title: 'Gestión de Pagos',
      subtitle: 'Historial y registro de pagos'
    },
    '/membresias-clientes': {
      title: 'Membresías de Clientes',
      subtitle: 'Gestión de membresías activas'
    }
  };
  ...
  private updatePageTitle(url: string): void {
    ...
    if (route && this.pageTitles[route]) {
      ...
      this.currentPageSubtitle = 'Sistema de recepción';
    }
  }
